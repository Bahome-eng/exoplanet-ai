  ...
  0;
        ...
        const analyticsData = {
          searchHistory: dbPredictions.map((result, index) => ({
            date: new Date(
              result.timestamp || Date.now() - index * 24 * 60 * 60 * 1000
            ).toISOString().split('T')[0],
            predictionNumber: index + 1,
            exoplanet_id: result.exoplanet_id || result.candidate_id || 'Unknown'
          })),
          monthlyStats: {
        ...
        const exoplanetsFound = results.filter(result =>
          result.prediction?.is_exoplanet || result.is_exoplanet
        ).length;
...
>
  ...
  formatter={(value, name, props) => [
    props.payload.exoplanet_id,
    'Candidate'
  ]}
/>
                  <Line 
  type="monotone" 
  dataKey="predictionNumber" 
  stroke="#4a9eff" 
  strokeWidth={3}
  dot={{ fill: '#4a9eff', strokeWidth: 2, r: 6 }}
activeDot={{ r: 8 }}
/>
                </LineChart>
              </ResponsiveContainer>
