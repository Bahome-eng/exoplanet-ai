...
function ManualPredict({ onSearchResult }) {
  ...
  const handlePredict = async () => {
    ...
    try {
      const response = await fetch('http://127.0.0.1:5000/api/predict/manual', {
        method: 'POST',
        headers: {
      ...
      if (data.prediction) {
        ...
        const statusText = data.prediction.is_exoplanet
          ? 'is an exoplanet'
          : 'is not an exoplanet';

        toast.success(
          `Prediction complete! ${confidencePercentage}% confident that the input ${statusText}.`
        );
      } else {
